...
int main() {
    ...
    TContainer* my_TContainer;
    std::shared_ptr<Figure> my_figure = nullptr;

    my_TContainer = new TContainer();
    ...
    while (command != 'q'){
            ...
            if (command == 'a') {
                ...
                std::cout << "The size of your vector is " << my_TContainer->Size() << std::endl;
                 ...
                 std::cout << "Type in the fields of which you want to clense the vector:" << std::endl;
                std::cin >> command;
                if (command == 's'){
                    std::cout << "Deleting by the square. Print the desired value:" << std::endl;
                    double searchsquare;
                    std::cin >> searchsquare;
                    my_TContainer->del(searchsquare);
                }
                else if (command == 't'){
                    std::cout << "Deleting by the figure type. Print the desired type:" << std::endl;
                    std::cin >> command;
                    if (command == 'p'){
                        std::cout << "You chose to remove all pentagons!" << std::endl;
                    }
                    if (command == 'h'){
                        std::cout << "You chose to remove all hexagons!" << std::endl;
                    }
                    if (command == 'o'){
                        std::cout << "You chose to remove all octagons!" << std::endl;
                    }
                        my_TContainer->del(command);
                    }
                else {
